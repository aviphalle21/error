...
$totalRevenue = $pdo->query("SELECT COALESCE(SUM(amount), 0) FROM payments WHERE payment_status = 'PAYMENT_VERIFIED'")->fetchColumn();
$monthRevenue = $pdo->query("SELECT COALESCE(SUM(amount), 0) FROM payments WHERE payment_status = 'PAYMENT_VERIFIED' AND MONTH(payment_date) = MONTH(CURRENT_DATE) AND YEAR(payment_date) = YEAR(CURRENT_DATE)")->fetchColumn();

// 4. Active Bookings
$activeBookings = $pdo->query("SELECT COUNT(*) FROM bookings WHERE booking_status IN ('ACTIVE', 'CONFIRMED')")->fetchColumn();
...
$monthlyQuery = $pdo->query("
    ...
    FROM payments 
    WHERE payment_status = 'PAYMENT_VERIFIED' AND YEAR(payment_date) = YEAR(CURRENT_DATE)
...
$planDataQuery = $pdo->query("
    ...
    WHERE us.subscription_status IN ('ACTIVE', 'CONFIRMED')
